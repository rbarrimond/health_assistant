...
class TestSchemaFieldImplementation:
    ...
    def test_power_zone_boundaries_computed(self, sample_fit_file, mocker):
        ...
        for power in [200, 220, 250, 280, 300] * 100:  # 500 data points
            record = MagicMock()
            record.get = MagicMock(side_effect=lambda k, p=power: {
        ...
        def get_messages(msg_type):
            if msg_type == 'session':
                ...
                profile = MagicMock()
                profile.get = MagicMock(side_effect=lambda k: {
        ...
        mocker.patch('FitParser.fit_parser.fitparse.FitFile', return_value=mock_fit)
        mocker.patch('FitParser.adapter.fitparse.FitFile', return_value=mock_fit)
    ...
    def test_aerobic_efficiency_metrics_computed(self, sample_fit_file, mocker):
        ...
        for i in range(300):
            record = MagicMock()
            record.get = MagicMock(side_effect=lambda k: {
    ...
    def test_resting_hr_extraction(self, sample_fit_file, mocker):
        ...
        profile_msg.get = MagicMock(side_effect=lambda key: {
