...
- Populate extended canonical telemetry fields during FIT extraction (temperature, respiration rate, left/right balance). RR interval now comes from HRV messages only; bump ingest version to v10.0.2.
- Restore Apple workout type extraction in canonical session metadata; bump ingest version to v10.0.3.
- Use FIT file_id "number" for activity ID resolution and add safe field access to avoid KeyErrors; bump ingest version to v10.0.4.
- Standardize FIT field access on `get_value()` and use `local_timestamp` consistently for activity local time; bump ingest version to v10.0.5.
- **BREAKING CHANGE**: Remove canonical laps parquet and `canonical_laps_blob` from Workouts. Laps are now stored only in `laps.json` (raw FIT lap messages) and returned as-is by lap endpoints. Bump ingest version to v10.0.0 and ingestion schema to 7.0.0.
- **BREAKING CHANGE**: Remove `parse()` method and all metric computation code (~536 lines) from FitParser to enforce Canonical Data Architecture v2.2.0 principle: "Canonical Parquet stream is the single source of metric truth." FitParser is now a pure artifact extractor that only calls `extract_canonical_records()`, `extract_metadata_messages()`, `extract_raw_fit_json()`, and `extract_fit_analysis()`. All metric computation (zones, TSS, IF, NP, VI, efficiency, decoupling) now occurs exclusively in `CanonicalAnalyticsEngine.from_dataframe()` which operates on `canonical.parquet` at read time with lazy-evaluated `@computed_field` properties. This architectural cleanup eliminates ~400 lines of duplicated computation logic, removes numpy and Config dependencies from ingestion, and enables recomputability—metrics can be recalculated with improved algorithms without re-ingesting FIT files. Migration: Any code calling `parser.parse()` must migrate to canonical extraction methods + CanonicalAnalyticsEngine. Bump ingest version to v9.0.0 for breaking API change.
- Add a message-type index in FitParser to speed repeated lookups for metadata, laps, timezone inference, and device collision checks.
- Remove the thin `fit_message_utils` wrapper and call fitdecode directly in FitParser to reduce ingestion indirection.
- Stop slicing per-lap record payloads in FitParser; laps.json now remains a pure lap-message artifact while records stay exclusively in canonical parquet.
- Remove FitParser message/field helper wrappers in favor of direct fitdecode `FitDataMessage.get_value()`/`get_raw_value()` access; FitStructureAnalyzer now consumes fitdecode messages directly. Bump ingest version to v9.0.2.
